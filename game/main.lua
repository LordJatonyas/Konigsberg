function love.load()
	...
	-- bridges
	rail_pos_x = {
	{11/160,137/160,11/32,11/32},
	{},
	{11/32,11/32,111/160},
	{21/80,47/80,47/80},
	{3/16,59/80,53/160,53/160},
	...
	{23/160,23/160,9/32,9/32,57/80,57/80,13/32},
	...
	{1/5,2/3,1/5,2/3,1/5,2/3,9/20},
	{23/120,79/120,23/120,79/120,9/20}
	}
	
	rail_dimen = {
	{{3/40,5/12},{3/40,5/12},{5/16,0.1},{5/16,0.1}},
	{},
	{{19/80,0.1},{19/80,0.1},{3/40,11/60}},
	{{1/8,0.1},{1/8,0.1},{1/8,0.1}},
	{{3/40,17/60},{3/40,17/60},{27/80,0.1},{27/80,0.1}},
	...
	{{3/40,2/15},{3/40,2/15},{3/40,2/15},{3/40,2/15},{3/40,2/15},{3/40,2/15},{3/16,0.1}},
...
function love.mousepressed(x,y,button,istouch)
	...
	if button == 1 and ((level >= 3 and level < 22) or level == 23) and picked_landmass then
		...
		for d = 1,#rail_pos_x[level] do
			...
			if rail_dimen[level][d][2] == 0.1
			and x > rail_pos_x[level][d]*width and x < rail_pos_x[level][d]*width + rail_dimen[level][d][1]*width
			...
			then
				...
				if rail_pos_y[level][d] > start_pos[2] and rail_pos_y[level][d] + rail_dimen[level][d][2] < start_pos[2] + start_glow_dimen[2] then
			...
			elseif rail_dimen[level][d][1] == 3/40
			and x > rail_pos_x[level][d]*width + 5 and x < rail_pos_x[level][d]*width + rail_dimen[level][d][1]*width - 5
			and y > rail_pos_y[level][d]*height and y < rail_pos_y[level][d]*height + rail_dimen[level][d][2]*height
			then
				-- check if x starting and ending x coordinates of bridge lies within the highlighted landmass
				if rail_pos_x[level][d] > start_pos[1] and rail_pos_x[level][d] + rail_dimen[level][d][1] < start_pos[1] + start_glow_dimen[1] then
	...
	if level == 22 and button == 1 then
		...
		and y >= rail_pos_y[level][3]*height and y <= rail_pos_y[level][3]*height + rail_dimen[level][3][2]*height
		then
			break1 = true
			-- sounds['crack']:play()
		end
		if x >= rail_pos_x[level][4]*width + 5 and x <= rail_pos_x[level][4]*width + rail_dimen[level][4][1]*width - 5
		and y >= rail_pos_y[level][4]*height and y <= rail_pos_y[level][4]*height + rail_dimen[level][4][2]*height
		then
			break2 = true
			-- sounds['crack']:play()
		end
